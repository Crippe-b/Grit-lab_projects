...
func CollectAllAgruments(arguments []string) (structures.Flags, []string, []string, []string, bool) {
	...
	for i := 0; i < len(inputArgs); i++ { // Collecting flags
		if inputArgs[i][:1] == "/" {
		...
		if strings.Contains(inputArgs[i], "/") { // Collecting paths
		...
		if !strings.Contains(inputArgs[i], "-") || inputArgs[i] == "-" { // Collecting files and folders
